...
`.trim()
        ...
        sendUrl: {
            value(jid, quoted, options) {
                let content = generateWAMessageFromContent(jid, { extendedTextMessage: { ...options }}, quoted)
                return conn.relayMessage(jid, content.message, { messageId: content.key.id })
            }
        },
        resize: {
        	value(buffer, quality1, quality2) {
        	return new Promise(async (resolve, reject) => {
                var baper = await Jimp.read(buffer)
                var ab = await baper.resize(quality1, quality2).getBufferAsync(Jimp.MIME_JPEG)
                resolve(ab)
            })
            }
        },
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        mtype: {
            get() {
                ...
                return Object.keys(this.message)[0]
            },
            enumerable: true
        },
        msg: {
        ...
        quoted: {
            get() {
                ...
                const quoted = contextInfo?.quotedMessage // contextInfo?.quotedMessage.viewOnceMessageV2 ? contextInfo?.quotedMessage.viewOnceMessageV2.message : contextInfo?.quotedMessage
                if (!msg || !contextInfo || !quoted) return null
                const type = Object.keys(quoted)[0] // getContentType(quoted)
                let q = quoted[type]
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
                    ...
                    text: {
                        get() {
                            return text || this.caption || this.contentText || this.selectedDisplayText || this?.message?.imageMessage?.caption || this?.message?.videoMessage?.caption || ''
                        },
                        enumerable: true
                    },
                    mentionedJid: {
        ...
        text: {
            get() {
                const msg = this.msg
                const text = (typeof msg === 'string' ? msg : msg?.text) || msg?.caption || msg?.contentText || msg?.message?.imageMessage?.caption || this?.message?.videoMessage?.caption || ''
...
export function protoType() {
   ...
   Number.prototype.toTimeString = function toTimeString() {
      return ems(this, { shortFormat: true })
   }
